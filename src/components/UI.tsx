...
export function NextProject({ label, to }: { label: string; to: string }) {
  return <Link className="next-project" to={to}><span>下一个项目 / NEXT</span><strong>{label}</strong><i>→</i></Link>
}
...
export function CaseHero({ title, subtitle, claim, summary, tags, tone, children }: { title: string; subtitle: string[]; claim?: string; summary: string; tags: string[]; tone: string; children: ReactNode }) {
  return <section className={`case-hero tone-${tone}`}><div className="mesh" aria-hidden="true" /><div className="container case-hero-grid"><div className="case-copy"><p className="eyebrow">案例研究 / CASE STUDY · 2026</p><h1>{title}</h1><p className="case-subtitle">{subtitle.map(x => <span key={x}>{x}</span>)}</p>{claim && <p className="case-claim">{claim}</p>}<p className="case-summary">{summary}</p><Tags items={tags} /></div><div className="case-visual">{children}</div></div></section>
}
