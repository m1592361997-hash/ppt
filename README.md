...
- `#/work/personal-ppt`：个人 PPT 项目展示（6 套真实作品、118 张完整页面）
...
当前已收录 6 套真实 PPT，共 118 张页面。图片按项目分别放在 `01-copenhill` 至 `06-momo` 六个子目录中，并转换为最大 1920×1080 的高质量 WebP。项目标题、说明、标签、封面路径和完整页面路径集中在 `src/data/portfolio.ts` 的 `personalPpt.works` 中。

后续替换某套作品时，保持 `slide-01.webp`、`slide-02.webp` 的顺序命名，并同步修改数据文件中的页面数量即可。
...
WPS AI PPT 竞品分析是基于六款产品公开版本完成的个人独立测试，不代表 WPS 官方结论。页面中的 54 次测试、评分与完成时间均来自本次统一任务测试；结果只适用于当时的产品版本、账号权限、测试材料和设备环境。产品功能与权益会变化，正式投递前建议保留测试记录并复核版本信息。个人 PPT 项目已使用真实作品页面；个人设计项目的 3 个作品位仍是待替换内容，不得把占位内容直接作为真实投递证据使用。
